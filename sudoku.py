...
def find_possible_values(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.Set[str]:
    ...
    num_row = set(get_row(grid, pos))
    num_col = set(get_col(grid, pos))
    num_block = set(get_block(grid, pos))
    return numbers - num_row - num_col - num_block
...
def solve(grid: tp.List[tp.List[str]]) -> tp.Optional[tp.List[tp.List[str]]]:
    ...
    for val in possible_values:
        grid[empty_positions[0]][empty_positions[1]] = val
...
def check_solution(solution: tp.List[tp.List[str]]) -> bool:
    ...
    for i in range(9):
        for j in range(9):
            ...
            if len(set(row)) != len(row) or '.' in row:
                return False
            if len(set(col)) != len(col) or '.' in col:
                return False
            if len(set(block)) != len(block) or '.' in block:
                return False
    return True
